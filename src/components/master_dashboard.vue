            ...
            <div class="w-full overflow-y-auto overflow-x-hidden px-2 mt-6">
                            ...
                            <div class="card-title d-flex flex-column">
                                <div class="d-flex align-items-center">
                                    <span class="fs-4 fw-semibold text-gray-500 me-1 align-self-start"
                                        v-text="currency.symbol"></span>
                                    <span class="fs-2hx fw-bold text-gray-900 me-2 lh-1 ls-n2"
                                        v-text="Number(content.total_invoices_amount).toFixed(2)"></span>
                                </div>
                                <span class="text-gray-500 pt-1 fw-semibold fs-6"
                                    v-text="translate('Total invoices amount')"></span>
                            </div>
                        </div>
                        ...
                        <div class="px-4 pt-2 pb-4 d-flex align-items-center">
                            ...
                            <div class="d-flex flex-column content-justify-center w-100">
                                <div class="d-flex gap-4 fs-6 fw-semibold align-items-center"
                                    v-for="invoice in content.payment_methods_invoices_amount">
                                    <div class=" rounded-2  my-3"><img class="w-10 h-10"
                                            :src="'/uploads/img/payment_methods/' + invoice.payment_method.toLowerCase() + '.png'" />
                                    </div>
                                    <div class="text-gray-500 flex-grow-1 me-4" v-text="invoice.payment_method"></div>
                                    <div class="fw-bolder text-gray-700 text-xxl-end"
                                        v-text="currency.symbol + '' + Number(invoice.value).toFixed(2)"></div>
                                </div>
                            </div>
                        </div>
                    </div>
                                ...
                                <div class="d-flex align-items-sm-center mb-7" v-for="item in content.latest_audio">
                                    ...
                                    <div class="d-flex flex-row-fluid flex-wrap align-items-center">
                                        <div class="flex-grow-1 me-2">
                                            <a href="#" class="text-gray-800 fw-bold text-hover-primary fs-6" v-text="item.name"></a>

                                            <span class="text-muted fw-semibold d-block pt-1" ><vue-feather type="eye" /> <span v-text="item.views_count"></span> </span>
                                        </div>
                            ...
                            <div class="card-header pt-7">
                                <h3 class="card-title align-items-start flex-column">
                                    <span class="card-label fw-bold text-gray-800"
                                        v-text="translate('Product Orders')"></span>
                                    <span class="text-gray-500 mt-1 fw-semibold fs-6"
                                        v-text="translate('Latest orders from customers')"></span>
                                </h3>
                                <div class="card-toolbar">
                                    <div class="d-flex flex-stack flex-wrap gap-4">
                                        <div class="d-flex align-items-center fw-bold">
                                            <div class="text-gray-500 fs-7 me-2" v-text="translate('Status')"></div>
                                            <select v-model="orderStatus" :required="true"
                            ...
                            <div class="card-body pt-8">
                                <table class="table align-middle table-row-dashed fs-6 gy-3"
                                    ...
                                    <tbody class="fw-bold text-gray-600" v-if="content.latest_orders">
                                        ...
                                        <tr
                                            v-for="order in content.latest_orders.map(e => (orderStatus == 0 || e.status == orderStatus) ? e : null).filter(e => e)">
                                            <td>
                                                <span v-if="order" class="text-gray-800 text-hover-primary"
                                                    v-text="'#' + order.order_id"></span>
                                            </td>
                                            ...
                                            <td class="text-end">
                                                <span class="text-gray-600 text-hover-primary"
                                                    v-if="order && order.customer" v-text="order.customer.name"></span>
                                            </td>

                                            <td class="text-end" v-text="order ? order.total_amount : ''"> </td>

                                            <td class="text-end">
                                                <span v-if="order" v-text="translate(order.status)"
                                                    :class="orderStatusClass(order.status)"
                                                    class="badge py-3 px-4 fs-7 "></span>
                                            </td>
                ...
                <div class="w-full py-10">
                    ...
                    <div class="w-full bg-white p-4 mb-4 rounded-lg" v-if="content.orders_charts">
                        <dashboard_pie_chart v-if="content.orders_charts" type="bar" :key="content"
                            :options="getMixChartData(content.orders_charts, content.invoices_charts)" />
                    </div>
                </div>
                        ...
                        <div class="card-body w-full">
                            <div class="w-full" v-if="pie_options">
                                <dashboard_pie_chart v-if="pie_options" type="pie" :key="pie_options"
                                    :options="getPieChartData(content.top_visits, 'times')" />
                            </div>
                        </div>
                    </div>
