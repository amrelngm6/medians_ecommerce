...
async function initMap() {
  ...
  const mapMarkersList = JSON.parse(document.getElementById("map-markers").value);
  
  // The map, centered at Uluru
  map = new Map(document.getElementById("map"), {
    zoom: 3,
    center: { lat: mapMarkersList[0]['lat'], lng: mapMarkersList[0]['lng'] },
    mapId: "UomoMap",
  ...
  for (let i = 0; i < mapMarkersList.length; i++) {
    branches[i] = { lat: mapMarkersList[i]['lat'], lng: mapMarkersList[i]['lng'] } 
    branchesMarkers[i] = new Marker({
      map: map,
      position: branches[i],
      icon: "/uploads/img/map-marker.png",
      title: "London",
    });
  ...
  for (let i = 0; i < mapMarkersList.length; i++) {
    ...
    store_selector[i] = document.querySelector("#store_selector_"+(i+1));
    store_selector[i].addEventListener("click", function(e) {
    // jQuery(document).on("click", '.store_selector_', function(e) {
      store_info.classList.remove("hide");
      map.setCenter(branchesMarkers[jQuery(this).attr('data-index')].getPosition());
      map.setZoom(12);
      document.querySelector(".google-map__marker-detail__content").innerHTML = store_selector[i].closest('.store-location__search-result__item').innerHTML;
    }, false);
